...
- **C2 step 2（mock-first，已完成，提案创建从 builder 转到 comparator，保 1:1）**：
  ...
  - **gate transfer（安全链改动，orchestrator.py:347）**：加 `elif role==COMPARATOR` 分支，把 `output["chosen"]`（**不是** `{selected_index,...,chosen}` 审计 dict）传给 `_create_execution_proposal(role=COMPARATOR)`；gate 在 `chosen.proposed_files` 上触发。builder 仍留在 `(BUILDER, DOCUMENTATION)` 分支（wrapper→建 0，无害+防御）。**`_create_execution_proposal` 函数体未改**。
  - **1:1 在唯一 gate 站点保持**（execution_proposals 无 UNIQUE(task_id)，N 个提案=N 条可批准链=安全问题；唯一创建路径 = orchestrator.py 该 gate）：builder 0 + comparator 1（chosen）= 1。N-1 非选中：不建提案、**经 builder `agent_runs.raw_output`（全量 N wrapper）审计**、collapse 后从 runtime 丢弃。proposal.role = "comparator"（保留，审计真实）。documentation 仍建自己那 1 个（与往常一致）。
  - **未动**：`_create_execution_proposal` 体 / UNIQUE 链 / snapshot/request/freeze/confirm/dry-run/execute / MAX_REJECTIONS / idx 循环 / rejection rewind / 审批门 / blocking-veto / **model builder + BuilderOutputSchema（mock-first，真实模型产 N 推迟到子步骤）** / 4 个下游 reader（collapse 仍给单个 chosen）。
  - 测试：`tests/c2_step2_gate_transfer_test.py`（17 安全断言：builder=0 / comparator=1=chosen / total=1 / N-1 无痕 / chosen 正确 / 读 wrapper / collapse / raw_output 审计 / role=comparator）。合法 shape 更新：comparator_wiring（KnownBuilder→wrapper，f 翻转 builder0/comparator1）、phase3（builder 输出=wrapper）、phase6ea（proposal role builder→comparator）、rejection_feedback_removed（mock builder 措辞移进 proposals[0]）。**4 个下游 reader 字节未改**。Unit 29→30，回归 57→58 全绿。
  - **下一步 = 真实模型产 N（子步骤，待人工）**：改 builder prompt + BuilderOutputSchema 验证 wrapper + 逐提案 normalization。当前 step 2 是 mock-first，已跑通完整安全链转移。
