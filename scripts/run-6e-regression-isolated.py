#!/usr/bin/env python3
"""Phase 6E self-contained regression runner.

Starts a temporary server with isolated DB and port,
runs all 6E acceptance tests in sequence, then tears down.

Usage:
    python scripts/run-6e-regression-isolated.py
"""
...
def main():
    db_file = tempfile.mktemp(suffix=".db", prefix="6e_regression_")
    env = os.environ.copy()
    env["RUNTIME_DB"] = db_file
    env["RUNTIME_PORT"] = str(PORT)

    print("=" * 60)
    print("  Phase 6E Regression Suite (isolated)")
    print("=" * 60)
    print(f"  DB:   {db_file}")
    print(f"  Port: {PORT}")
    print()
    ...
    print()
    print("-" * 60)
    print(f"  6E Total: {total}  Pass: {passed}  Fail: {failed}")
    print("-" * 60)
